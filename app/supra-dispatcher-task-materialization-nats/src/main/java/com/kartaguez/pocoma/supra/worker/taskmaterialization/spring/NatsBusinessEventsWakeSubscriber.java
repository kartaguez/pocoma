...
public class NatsBusinessEventsWakeSubscriber implements SmartLifecycle {
	...
	private final TaskMaterializationNatsProperties properties;
	...
	void onMessage(byte[] payload) {
		try {
			TaskMaterializationWakePayload wakePayload = objectMapper.readValue(payload, TaskMaterializationWakePayload.class);
			...
			LOGGER.log(System.Logger.Level.WARNING, "Ignoring invalid NATS task materialization wake payload", exception);
		}
	}
}
