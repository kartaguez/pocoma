...
public final class SourceVersionWatermarkConsumptionLocator implements ConsumptionLocator {
	public static final String CONSUMER_TYPE = "SOURCE_VERSION_WATERMARK";

	private final WorkerSegment segment;
	private final SourceVersionWatermarkEventDiscoveryPort discovery;
	private final EventPort events;
	private final ObserveSourceVersionUseCase observe;
	private final ConsumptionTechnicalFailureClassifier failureClassifier;
	private final Clock clock;

	public SourceVersionWatermarkConsumptionLocator(WorkerSegment segment,
			SourceVersionWatermarkEventDiscoveryPort discovery, EventPort events,
			ObserveSourceVersionUseCase observe, ConsumptionTechnicalFailureClassifier failureClassifier, Clock clock) {
		...
		this.observe = requireNonNull(observe, "observe must not be null");
	...
	private ConsumptionExecutionResult execute(UUID eventId, UUID slotId) {
		...
		observe.observe(new ObserveSourceVersionInput(event.potId(), event.version(), clock.instant()));
