...
final class ComputePotBalancesService implements ComputePotBalancesUseCase {
	...
	ComputePotBalancesService(
		...
		this.potShareholdersPort = Objects.requireNonNull(potShareholdersPort, "potShareholdersPort must not be null");
	...
	public PotBalances computePotBalancesFull(PotId potId, long targetVersion) {
		...
		Map<ShareholderId, Balance> zeroBalances = potShareholdersPort.loadActiveAtVersion(potId, targetVersion)
				.shareholders()
				.keySet()
				.stream()
				.collect(Collectors.toMap(shareholderId -> shareholderId, shareholderId -> new Balance(
						shareholderId,
						Fraction.ZERO)));
		Map<ShareholderId, Balance> expenseBalances = potBalancesCalculator.calculateExpensesBalances(
				potId,
				projectedExpensePort.loadActiveAtVersion(potId, targetVersion));
		PotBalances targetBalances = new PotBalances(
				potId,
				targetVersion,
				BalanceMapOperations.add(zeroBalances, expenseBalances));

		potBalancesPort.saveFull(targetBalances);
		return targetBalances;
	}
}
