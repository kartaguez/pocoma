...
@Component
public class JpaSourceVersionWatermarkEventDiscoveryAdapter implements SourceVersionWatermarkEventDiscoveryPort {
	private static final int PAGE_SIZE = 128;
	private final JpaEventConsumptionDiscoveryRepository repository;

	public JpaSourceVersionWatermarkEventDiscoveryAdapter(JpaEventConsumptionDiscoveryRepository repository) {
	...
	public Optional<EventConsumptionCandidate> findNextEligibleCandidate(WorkerSegment segment, Instant now,
		...
		while (true) {
			var page = repository.findNextEligibleForSourceVersionWatermark(now, cursor, PAGE_SIZE);
