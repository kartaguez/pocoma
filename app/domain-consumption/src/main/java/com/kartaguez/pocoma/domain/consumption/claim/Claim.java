...
public record Claim(
	...
	public Claim endAt(Instant instant) {
		...
		return new Claim(claimId, consumptionKey, token, claimedBy, claimedAt, leaseUntil,
				invalidatedAt, Optional.of(instant), Optional.empty());
	}

	public Claim failAt(Instant instant, ProcessingFailure processingFailure) {
		requireNonNull(instant, "instant must not be null");
		requireNonNull(processingFailure, "processingFailure must not be null");
		return new Claim(claimId, consumptionKey, token, claimedBy, claimedAt, leaseUntil,
				invalidatedAt, Optional.of(instant), Optional.of(processingFailure));
	}

	public Claim invalidateAt(Instant instant) {
		requireNonNull(instant, "instant must not be null");
		return new Claim(claimId, consumptionKey, token, claimedBy, claimedAt, leaseUntil,
				Optional.of(instant), endedAt, failure);
	}
}
