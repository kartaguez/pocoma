...
public final class ExecuteProjectionTaskService {
	private final ProjectionTaskPreparation preparation;
	...
	public ExecuteProjectionTaskService(ProjectionTaskPreparation preparation, ProjectionWritePort projections,
			FinalizeConsumptionUseCase finalizer, HandleConsumptionFailureUseCase retryHandler, Clock clock) {
		this.preparation = requireNonNull(preparation); this.projections = requireNonNull(projections);
		this.finalizer = requireNonNull(finalizer); this.retryHandler = requireNonNull(retryHandler);
		this.clock = requireNonNull(clock);
	}

	public ProjectionTaskExecutionResult execute(ProjectionTask task, Claim claim) {
		requireNonNull(task, "task must not be null"); requireNonNull(claim, "claim must not be null");
		var key = task.projectionKey();
		var outcome = requireNonNull(preparation.prepare(key), "preparation outcome must not be null");
		FencedMutationResult result;
		if (outcome instanceof Prepared prepared) {
			if (!prepared.projection().projection().projectionKey().equals(key)) {
				throw new ProjectionPreparationInvariantViolationException(key,
						"validated projection has another ProjectionKey", null);
			}
