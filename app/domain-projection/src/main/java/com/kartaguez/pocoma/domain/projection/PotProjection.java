...
public record PotProjection(ProjectionIdentity identity, PotProjectionStatus status, String label,
	...
	public PotProjection {
		requireNonNull(identity); requireNonNull(status); requireNonNull(label); requireNonNull(creatorId);
		requireNonNull(shareholders); requireNonNull(expenses);
		...
		if (shareholders.stream().map(PotProjectionShareholder::shareholderId).distinct().count() != shareholders.size())
		...
		var ids = shareholders.stream().map(PotProjectionShareholder::shareholderId).collect(java.util.stream.Collectors.toSet());
		for (var expense : expenses) {
			if (!ids.contains(expense.payerId())) throw new IllegalArgumentException("expense payer is not a shareholder");
			if (expense.shares().stream().anyMatch(s -> !ids.contains(s.shareholderId())))
