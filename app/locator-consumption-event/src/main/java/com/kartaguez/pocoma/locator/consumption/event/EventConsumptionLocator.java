...
import com.kartaguez.pocoma.orchestrator.consumption.ConsumptionLocator;
import com.kartaguez.pocoma.orchestrator.consumption.ConsumptionSearch;
import com.kartaguez.pocoma.orchestrator.consumption.LocatedConsumption;
...
public final class EventConsumptionLocator implements ConsumptionLocator {
	...
	private final ConsumptionFailureClassifier failureClassifier;

	public EventConsumptionLocator(PipelineDefinition pipeline, WorkerSegment segment, EventPort events,
			CreateTasksForEventUseCase createTasks, ConsumptionFailureClassifier failureClassifier) {
	...
	private final class Search implements ConsumptionSearch {
		...
		public Optional<LocatedConsumption> next() {
			...
			return Optional.of(new LocatedConsumption(key(event), context -> execute(event, context.slotId()),
					failureClassifier));
		}
	}

	private ConsumptionExecutionResult execute(RecordedEvent<? extends BusinessEvent> event, java.util.UUID slotId) {
		...
		List<ConsumptionResult> results = creation.tasks().stream()
