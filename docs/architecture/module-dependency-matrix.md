...
| `engine-query` | six lectures Pot/balances legacy encore branchées au primaire ; contrats framework-free CURRENT/EXACT, vues, producteurs serving fournis, readiness/latest-known read-only et enveloppe versionnée | Pot, policies, balance, projection, pipeline, core | read-projection engine, command processing, consumption, workers, frameworks | transition read path — contrats 7.9.1 livrés, resolver/intégration Lots 7.9.2 à 7.13 |
